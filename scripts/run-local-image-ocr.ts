...
const workerId = process.env.MATERIAL_PROCESSING_WORKER_ID || 'local-ocr-worker:dev';
const endpoint = process.env.LOCAL_OCR_ENDPOINT || '';
const processorVersion = process.env.LOCAL_OCR_PROCESSOR_VERSION || '';
const claimLimit = 3;
...
async function processJob(
  ...
  if (
    ...
    || file.status !== 'ready'
    || !IMAGE_TYPES.has(file.file_type)
    || file.file_size < 12
    || file.file_size > MAX_OCR_IMAGE_BYTES
  ) {
    const failed = await failJob(job, 'invalid_ocr_source', false);
  ...
  if (
    image.byteLength !== file.file_size
    || !validateFileSignature(Buffer.from(image), file.file_type)
  ) {
    const failed = await failJob(job, 'ocr_signature_or_size_mismatch', false);
    return failed === 'lease_lost' ? 'lease_lost' : 'failed';
  }

  try {
    const extracted = await extractTextWithLocalOcr(image, {
      endpoint,
    ...
    const code = error instanceof LocalOcrError
      ? error.code
      : 'unexpected_ocr_processing_failure';
    const retryable = !(error instanceof LocalOcrError)
...
async function main() {
  if (!execute) {
    console.log(JSON.stringify({
      ...
      storageReads: false,
      supportedJobTypes: ['image_ocr'],
      message: 'Use --execute only with an approved private OCR service.',
    }));
    return;
  }
  ...
  if (
    ...
    throw new Error('Invalid local OCR worker configuration');
  }

  const { data, error } = await adminSupabase.rpc('claim_material_processing_jobs', {
    p_worker_id: workerId,
    p_job_types: ['image_ocr'],
    p_limit: claimLimit,
    p_lease_seconds: leaseSeconds,
  });
  if (error || !Array.isArray(data)) {
    throw new Error('Unable to claim image OCR jobs');
  }
  ...
  console.log(JSON.stringify({
    event: 'material_processing.image_ocr_batch_completed',
    ...summary,
  }));
}
...
main().catch((error) => {
  console.error(JSON.stringify({
    event: 'material_processing.image_ocr_batch_failed',
