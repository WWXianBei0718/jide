...
create table if not exists public.external_api_usage_events (
  ...
  operation text not null check (operation in ('voice_clone', 'tts')),
  units integer not null default 1 check (units > 0 and units <= 5000),
  created_at timestamp with time zone not null default now()
);

create index if not exists idx_external_api_usage_user_operation_created
  on public.external_api_usage_events (user_id, operation, created_at desc);

alter table public.external_api_usage_events enable row level security;

revoke all on table public.external_api_usage_events from anon, authenticated;
...
begin
  ...
  if requested_units is null or requested_units < 1 or requested_units > 5000 then
