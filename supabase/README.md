...
17. `202608060000_add_message_feedback.sql` 新增仅允许评价自己人物下 AI 回复的“像 / 不像”反馈表、RLS、账号删除和导出边界；反馈不会自动写入人物资料或向量记忆。
18. `202608270000_scope_memory_retrieval_to_embedding_model.sql` 为向量检索加入 Embedding 模型条件，防止切换 OpenAI、千问或后续模型后错误比较不同向量空间。
