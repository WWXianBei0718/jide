...
| RLS 部署漂移 | 12 份版本化迁移已有校验清单和自动策略测试，且均已部署到开发库；第 12 份已回读确认 RLS、登录客户端只读权限和 Service Role 函数边界；尚无远程 CI 与生产 Security Advisor 门禁 | 中 | 接入私有远程仓库 CI；保持双用户正反向测试并加入生产发布检查 |
