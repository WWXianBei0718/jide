...
4. **高权限边界**：普通业务读写必须使用用户作用域客户端和 RLS。Service Role 只允许出现在经过认证、资源归属校验的受控编排点（例如私有对象清理、账号导出/删除、保存供应商回复）或隔离后台任务，并由静态白名单约束。
...
| 风险 | 当前证据 | 级别 | 目标措施 |
|---|---|---:|---|
| Service Role 使用面扩大 | 普通 CRUD 已使用用户作用域客户端；高权限客户端仍用于私有对象编排、导出/删除、索引和可信 AI 回复写入 | 中 | 用静态白名单锁定导入点；每个高权限查询继续显式绑定已认证用户/人物；长期迁移到受审计任务 |
| 浏览器绕过业务 API | 核心表读写已进入认证 API；浏览器仅保留 Supabase Auth 和服务端签发后的短效隔离区上传 | 低 | 保持自动检查，禁止客户端包导入管理客户端或直接读写核心表 |
| 声音样本不安全传输或遗留 | 声音样本先进入私有隔离链路，完成基础签名校验后由服务端以 multipart 发送；成功后清理原样本 | 中 | 接入恶意文件扫描 Worker 和异步重试队列；生产闸门通过前保持关闭 |
| 声纹同意未强制 | 每份样本要求明确声明，调用前写入带版本、告知哈希和证据的 `consents`；写入失败会中止 | 中 | 增加撤回、监护/代理授权和同意记录端到端验收 |
| 文件扫描链路不完整 | 私有隔离区、状态机、Storage RLS、签名校验和正式区发布已完成；恶意文件扫描 Worker 未接入 | 高 | Worker 扫描通过后才发布，生产不得启用未扫描文件 |
| 应用级速率限制/额度不足 | 聊天、上传、声音克隆和 TTS 已有数据库原子配额；匿名调用被拒绝 | 中 | 上线前补 IP/设备级滥用保护、监控和套餐策略 |
| 安全审计缺少远程告警 | 关键 API 已写入不含正文/用户标识的 `security_audit_events`，并保留结构化控制台降级日志；第 11 份迁移已部署，90 天保留工具已完成但尚未定时运行，也没有远程看板与告警 | 中 | 将保留工具接入受控定时任务；建立错误率和供应商失败告警 |
| 删除闭环尚未真实验收 | 账号/人物删除已编排供应商声音、私有 Storage、数据库和 Auth；失败不会误报成功 | 高 | 用专用测试账号做破坏性端到端验收，增加重试任务和供应商删除回执 |
| RLS 部署漂移 | 16 份版本化迁移已有校验清单和自动策略测试并全部部署；第 16 份已回读确认登录用户不能读取扫描租约或调用扫描函数，扫描函数仅 Service Role 可执行；尚无远程 CI 与生产 Security Advisor 门禁 | 中 | 接入私有远程仓库 CI；保持双用户正反向测试并加入生产发布检查 |
...
| 本地 PDF 解析器处理恶意文档 | 使用固定 PDF.js 版本；限制输入、页数和输出；关闭求值、图片解码、外部字体、WASM；Worker 默认不写入，错误只保存固定代码 | 高 | 保持隔离 Worker 和资源限制；完成依赖漏洞审查与恶意 PDF 样本测试后再允许处理真实文件 |
| 恶意文件扫描服务未部署 | 私有 ClamAV 协议客户端、隔离区 Worker、租约、重试、干净后发布和感染后拒绝边界已实现；第 16 份迁移已部署并回读，默认演练不读取文件 | 严重 | 部署并更新 ClamAV 病毒库，完成 EICAR、超时和故障恢复验收后才允许生产文件发布 |
| 供应商保留和跨境风险未闭环 | OpenAI 对话/Embedding 已加入版本化、可撤回的按人物授权门禁；TTS 会复核当前声音处理授权；告知文本仍是产品工程版本，尚未完成 PIPIA/DPIA、供应商协议、区域和默认保留确认 | 严重 | 法律复核告知；完成 PIPIA/DPIA、DPA、区域评估、ZDR/最短保留、退出与删除流程 |
...
当前采用混合方式：浏览器持有 Supabase 用户会话用于认证，并把访问令牌发送给本项目 API；核心表读写由 API 中的用户作用域客户端执行。浏览器只在取得服务端签发的短效权限后直传隔离区文件。长期有两种可接受方式：
...
- `admin-supabase.ts`：使用 Service Role，只允许受控对象编排、账号导出/删除、索引、可信供应商回复写入和后台任务使用。导入点由自动测试白名单锁定。

普通 CRUD 即使已经在 BFF 验证归属，也必须使用用户作用域客户端，形成纵深防御。必须使用高权限客户端的操作，要先完成认证和资源归属校验，并在查询中再次绑定用户或人物范围。
...
- 声音克隆任务最终应异步化、幂等化；当前已移除 base64 JSON 直传，改为私有文件链路和服务端 multipart，但失败重试仍在同步请求内完成。

## 8. AI、RAG 与第三方处理安全

### 8.1 数据最小化

当前聊天接口已经先做用户/人物作用域的混合检索，在 8000 字符预算内选择排序最高的 3 个相关片段，并保存片段来源与检索版本。仍需继续完成：
...
2. 保持用户作用域与后台管理 Supabase 客户端分离；用自动白名单阻止 Service Role 扩散。
3. 保持核心表经 BFF 访问；继续统一错误格式、请求编号和高风险动作重新认证。
4. 保持版本化迁移、RLS、`WITH CHECK`、角色授权及正反向测试；接入远程 CI 和生产部署验真。
5. 私有 quarantine/assets、Storage RLS、短效上传凭证和基础签名校验已完成；补上恶意文件扫描 Worker。
6. 实现版本化同意；AI、声音和跨境处理在调用前强制验证。
7. 应用持久限流、CSP/安全头和生产诊断接口关闭已完成；补 CAPTCHA/IP 级滥用保护。
