...
| RLS 部署漂移 | 14 份版本化迁移已有校验清单和自动策略测试并全部部署到开发库；第 14 份已回读确认任务状态列可按人物读取、内部 Worker/租约列不可见、处理函数仅 Service Role 可执行；尚无远程 CI 与生产 Security Advisor 门禁 | 中 | 接入私有远程仓库 CI；保持双用户正反向测试并加入生产发布检查 |
