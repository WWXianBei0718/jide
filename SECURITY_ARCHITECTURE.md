...
| 安全审计未持久化 | 关键高成本 API 已使用不含正文/用户标识的请求编号和结构化安全日志，但尚无远程留存与告警 | 高 | 建立仅元数据、追加写、可告警的 `security_audit_events` 或受控日志平台 |
| 删除闭环尚未真实验收 | 账号/人物删除已编排供应商声音、私有 Storage、数据库和 Auth；失败不会误报成功 | 高 | 用专用测试账号做破坏性端到端验收，增加重试任务和供应商删除回执 |
| RLS 部署漂移 | 10 份版本化迁移已有校验清单和自动策略测试，并已全部部署到开发库；尚无远程 CI 与生产 Security Advisor 门禁 | 中 | 接入私有远程仓库 CI；保持双用户正反向测试并加入生产发布检查 |
