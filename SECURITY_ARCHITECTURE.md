...
| RLS 部署漂移 | 15 份版本化迁移已有校验清单和自动策略测试并全部部署；第 15 份已回读确认旧函数删除、新函数仅 Service Role 可调用且按 Worker 声明的任务类型过滤；尚无远程 CI 与生产 Security Advisor 门禁 | 中 | 接入私有远程仓库 CI；保持双用户正反向测试并加入生产发布检查 |
